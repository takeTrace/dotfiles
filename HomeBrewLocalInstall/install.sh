...
if [[ -d "${HOMEBREW_CACHE}" ]]; then
  ...
  # todo:  这里创建好 cache 目录后, 将缓存的东西拷贝过来. 之后的初始化安装就会安装缓存里的包, 不用重新下载了
  execute_sudo "$CP" "./HomeBrewCache/" "${HOMEBREW_CACHE}/"
fi
...
#   execute "git" "fetch" "origin" "--tags" "--force"
  # todo: 这里直接将现成的拷贝过来, 就不用从网上下载安装了, 达到离线安装的目的.
  execute_sudo "$CP" "./HomeBrewRepo/" "./"

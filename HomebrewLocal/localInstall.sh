...
localbrewdir="$(pwd)/$(dirname $0)"
echo "本地 home brew 位置: $localbrewdir"

read -r -p " 当前位置是否是  Homebrew Local 的位置? "
...
if [[ -d "${HOMEBREW_CACHE}" ]]; then
  ...
  execute_sudo "/bin/cp" -r ${localbrewdir}/Caches/Homebrew/* "${HOMEBREW_CACHE}/"
fi
...
(
  ...
  # 本地复制过来
  echo "本地复制过来"
  cp -r $localbrewdir/Homebrew/* "${HOMEBREW_REPOSITORY}/"
